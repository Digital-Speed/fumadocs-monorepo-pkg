...
export function init(
  ...
  options: any = {},
  element: Element | null = querySelector('docsy'),
  callback?: (e?: Error) => void,
) {
  if (element === null) {
    throw new Error('"element" argument is not provided and <docsy> tag is not found on the page');
  }
...
export function destroy(element: Element | null = querySelector('docsy')): void {
  if (element) {
...
export function hydrate(
  ...
  setTimeout(() => {
    ...
    hydrateRoot(element!, <Redoc store={store} />, {
...
/**
 * autoinit ReDoc if <docsy> tag is found on the page with "spec-url" attr
 */
function autoInit() {
  const element = querySelector('docsy');
  if (!element) {
    return;
  }
